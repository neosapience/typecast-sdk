...
public struct GenerateToFileRequest: Sendable {
  /// Voice ID in format 'tc_' followed by a unique identifier.
  /// Browse available API voices at https://typecast.ai/developers/api/voices.
  public var voiceId: String
  /// Text to convert to speech
  public var text: String
  /// Voice model to use. Defaults to SSFM v3.0.
  public var model: TTSModel
  public var language: LanguageCode?
  public var prompt: TTSPrompt?
  public var output: OutputSettings?
  ...
  public func toTTSRequest(fileURL: URL) -> TTSRequest {
    TTSRequest(
      voiceId: voiceId,
      text: text,
      model: model,
      language: language,
      prompt: prompt,
      output: output ?? Self.inferOutput(fileURL: fileURL),
      seed: seed
    )
  }

  private static func inferOutput(fileURL: URL) -> OutputSettings? {
    switch fileURL.pathExtension.lowercased() {
    case "mp3":
      return OutputSettings(audioFormat: .mp3)
    case "wav":
      return OutputSettings(audioFormat: .wav)
    default:
      return nil
    }
  }
